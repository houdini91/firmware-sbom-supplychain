...
| File | Role |
|---|---|
| [`policy/firmware.rego`](policy/firmware.rego) | **the gate** — 17 `verifier_reports`, ANDed into `allow`; emits the VSA predicate. Each report is tagged with the controls it satisfies. |
...
| [`fixtures/`](fixtures) | 17 gate-input **test vectors**: `clean.json` (ALLOW), `accepted-cve.json` (triaged ALLOW), and 15 negative fixtures each isolating one report's failure. Driven by [`../tests/run.sh`](../tests/run.sh). |
