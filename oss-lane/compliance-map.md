...
The release is hard-blocked on **31 always-emitted verifier reports** (32 on the clean demo). To keep one source of truth, they are **not
...
- **Prose + evidence mapping:** the "trust anchor" paragraph in [`../FRAMEWORKS.md`](../FRAMEWORKS.md) names all
  31 and the evidence atom (E1–E11) each consumes.
...
**and** the `slsa-provenance` verifier report asserts it so the VSA lists it. All 31 always-emitted reports are exercised in
