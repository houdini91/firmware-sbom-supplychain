...
| **E6** | **Policy verdict** | `oats.tech/policy-verdict/v0.1` (in-toto/DSSE) | the gate's verdict, as portable signed evidence | RATS-framed, OSCAL-shaped: `verifierReports[]` (19 observations) + **`controlAssessments[]`** (27 per-control findings across 6 frameworks — `satisfied`/`not-satisfied`/`not-applicable`) + `profiles.slsa-vsa.verifiedLevels:[L2]`. Backward-compatible with the SLSA-VSA shape | output artifact |
