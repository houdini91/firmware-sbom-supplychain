...
> is honestly MISSING_EVIDENCE and the demo never claims on-device detection; `deploy-time-reconcile` (SP 800-193
> §4.3.1, deploy-time/on-device — Track A) is emitted **only** when a device/image was CHIPSEC-scanned and the
> extracted per-module bytes reconcile (GUID-bound, bidirectional) against the same signed SBOM — absent on the
> clean demo (no device), but when PRESENT it **gates**: it requires **full coverage** (`matched == declared ==
> sbom.integrity.hashed`), a confirmed on-device byte swap / missing / unexpected module DENYs, and an
> **unverifiable** declared module (TE / non-PE / extraction error — a same-GUID PE→TE swap) DENYs and is named
> unless reviewed-exempt (`data.deploy_reconcile_exempt`, default empty) — not a benign skip; and the three CHIPSEC posture reports
...
| **§4.3.1** Detection of Corrupted Code | detect corruption vs an authorized reference | measured-boot measurement + golden RIM, on-device | **ADVISORY ◐** *(gate: `firmware-freshly-measured` + `deploy-time-reconcile`, both conditional)* | Two collection points now feed this: admission-time/off-device (`firmware-freshly-measured`, a fresh flash-time `FW_IMAGE` hash), and **deploy-time/on-device — `deploy-time-reconcile` (Track A): CHIPSEC extracts the deployed image's per-module bytes, our normalizer reproduces the base-0 hash, and they reconcile GUID-bound + bidirectional against the same signed SBOM** (122/122 on the OVMF reference), catching post-admission / flash-time drift. Both are absent on the offline demo (advisory-MISSING), gating when their evidence is present. **When present the deploy-time leg fails closed:** *clean* requires `matched == declared` **and** `declared == sbom.integrity.hashed` (full coverage — a 1-of-N reconcile does not pass), and an **unverifiable** declared module (TE / non-PE / unextractable / error — a same-GUID PE→TE swap) **DENYs and is named** unless it is a reviewed exemption in `data.deploy_reconcile_exempt` (default empty) — it is *not* a benign skip. A present verdict must be D-anchored (loose `image_digest == D`, else advisory-absent; under `REQUIRE_SIGNED_EVIDENCE=1` a present loose verdict with no signed bundle aborts). Even the deploy-time leg is CHIPSEC reading an image / live SPI — the on-device boot-time RTD (measured boot + golden RIM) remains futuristic; live-silicon SPI readback is roadmap A6. See [ADR 0001](docs/adr/0001-chipsec-is-a-deploy-time-collection-point.md), [DESIGN.md → Deploy-time reconcile](DESIGN.md), and [planning/CHIPSEC-INTEGRATION.md](planning/CHIPSEC-INTEGRATION.md). |
...
| **§8.4** Attestation Result | Verifier's signed verdict for the RP | E6 policy-verdict | **EVIDENCE (analog)** | The gate **is** the Verifier; E6 is a RATS-framed **Attestation Result** (OSCAL-shaped `controlAssessments`), over build-plane Evidence. Wire format is in-toto/DSSE, not EAT/CBOR. |
