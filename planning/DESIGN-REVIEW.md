...
the **enforcement** (19 OPA verifier reports → a signed SLSA VSA), the **fully-open stack** (cosign keyless +
...
3. **No framework/initiative layer.** ✅ **CLOSED (A2).** `initiatives/frameworks.yaml` +
   `verify-initiative.py` map the 19 verifier reports to framework control IDs and print per-framework,
...
- **Initiatives:** a declarative `framework → control → rule` manifest (SLSA / SSDF / 800-53 / 800-190 / CRA /
  BSI) mapping our 19 verifiers to real control IDs with "why this control" prose; fold `control_id →
...
- **Versioned rule catalog** (`ns/name@vN` + a metadata sidecar: help, labels, typed `inputs`, evidence
  requirement) and a **standard verdict schema** + shared rego lib, so the next verifier is boilerplate-free.
