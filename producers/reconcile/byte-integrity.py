...
# Shared FFS/PE carving + the reused normalizer, all in ffs.py so byte-integrity (at-rest) and
# deploy-reconcile (deploy-time) carve + canonicalize IDENTICALLY. pefile is re-exported here so
# `main()`'s `if pefile is None` guard and the unit tests (which reference this module's `pefile`)
# still resolve.
from ffs import (  # noqa: E402
    pe32_from_ffs, ffs_module_type, fmmt_extract,
    canon_unrebase, load_sbom_hashes, XIP_TYPES, pefile,
)
